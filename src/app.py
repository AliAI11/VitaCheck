# ============================================================================
# vitacheck — gradio app with groq llm meal plan generation
# deploy to huggingface spaces or run locally with: python app.py
# ============================================================================
import gradio as gr
import joblib
import pandas as pd
...
# load artifacts (paths work for both local and huggingface spaces)
ARTIFACT_DIR = os.path.join(os.path.dirname(__file__), "artifacts")

model = joblib.load(os.path.join(ARTIFACT_DIR, "xgb_tuned.joblib"))
scaler_obj = joblib.load(os.path.join(ARTIFACT_DIR, "scaler.joblib"))
target_le = joblib.load(os.path.join(ARTIFACT_DIR, "target_label_encoder.joblib"))

with open(os.path.join(ARTIFACT_DIR, "feature_info.json"), "r") as f:
    info = json.load(f)
...
# groq client — set GROQ_API_KEY as huggingface space secret
...
def generate_meal_plan(diagnosis, diet_type, allergies):
    ...
    if not groq_client:
        return "meal plan generation unavailable — groq api key not configured."
...
def predict(age, gender, bmi, smoking, alcohol, exercise, diet, sun,
    ...
    alcohol_map = {
        "none": "Unknown", "moderate": "Moderate",
        "heavy": "Heavy", "prefer not to say": "Unknown"
    }
...
.gradio-container {
    max-width: 1100px !important; margin: auto !important;
    background: #000000 !important; font-family: 'Inter', sans-serif !important;
    color: #d1d5db !important;
}
#header-row {
    background: #000000; border-radius: 16px; padding: 40px 36px 32px 36px;
    margin-bottom: 20px; border: 1px solid #1a2e1f; border-top: 3px solid #22c55e;
}
#header-row h1 { color: #4ade80 !important; font-size: 2.4em !important; margin-bottom: 4px !important; letter-spacing: -0.5px; }
#header-row p { color: #9ca3af !important; font-size: 15px !important; margin: 0 !important; line-height: 1.6 !important; }
#header-row em { color: #4b5563 !important; font-size: 13px !important; }
input, textarea, select, .gr-input, .gr-dropdown, .gr-slider input, input[type="text"], input[type="number"], input[type="range"] {
    background: #0a0a0a !important; border: 1px solid #1c1c1c !important;
    color: #d1d5db !important; border-radius: 8px !important;
}
input:focus, textarea:focus, select:focus {
    border-color: #22c55e !important; box-shadow: 0 0 0 2px rgba(34, 197, 94, 0.12) !important; outline: none !important;
}
.gr-button-primary, button.primary {
    background: #16a34a !important; border: none !important; border-radius: 10px !important;
...
.gr-button-primary:hover, button.primary:hover {
    background: #22c55e !important; box-shadow: 0 4px 16px rgba(34, 197, 94, 0.25) !important;
}
.section-title {
    color: #4ade80 !important; font-size: 12px !important; font-weight: 700 !important;
    text-transform: uppercase; letter-spacing: 2px; margin-bottom: 10px !important;
    padding-bottom: 8px; border-bottom: 1px solid #1a2e1f;
}
label, .gr-checkbox label, .gr-radio label { color: #9ca3af !important; font-size: 13px !important; }
.gr-label { background: #0a0a0a !important; border: 1px solid #1c1c1c !important; border-radius: 12px !important; }
...
.prose li, .prose p { color: #d1d5db !important; }
.prose a { color: #4ade80 !important; }
.tabs .tab-nav { border-bottom: 1px solid #1c1c1c !important; }
.tabs .tab-nav button { color: #6b7280 !important; font-weight: 600 !important; background: transparent !important; border: none !important; }
.tabs .tab-nav button:hover { color: #9ca3af !important; }
.tabs .tab-nav button.selected { color: #4ade80 !important; border-bottom: 2px solid #4ade80 !important; }
.disclaimer {
    background: #0a0f0a !important; border: 1px solid #1a2e1f !important;
    border-left: 3px solid #ca8a04 !important; border-radius: 8px !important;
    padding: 14px 18px !important; margin-top: 16px; color: #6b7280 !important; font-size: 13px !important;
}
.disclaimer strong { color: #ca8a04 !important; }
.block, .gr-panel, .gr-form, .gr-box, .gr-padded, .panel, .form, .wrap, .container {
    background: #000000 !important; border-color: #1c1c1c !important;
}
